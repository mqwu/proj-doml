...
########################################################################################
 ...
 for(generation in miniter:maxiter){
    ...
    recentgeneration<-2   # the number of recent generation for which all data will be used for training
    max_sample_size<-1000 # max # of samples in training set
    DataFormat<-c(3, X_dim, Y_dim)  # num of cols for index, x_dim, y_dim
    #############################################################
    ######  data reading ########################################
    ...
    # load recent generation data for training
    DataRecent<-NULL 
    ...
    if(generation>=m2 && generation>=miniter){
     ...
     if(length(E1)>0 && length(E2)>0){ 
         ...
         ## generate training data = recent data + sampling from old generation
     ...
     for(k in (sum(DataFormat[1:2])+1):sum(DataFormat)){  # y cols
